...
export class NhanVienDetailComponent implements OnInit {
  ...
  public uploadAvatar(){
    ...
    this.nvService.uploadAvatar(formData, this.nv.id).subscribe(data=>{
      this.avatar = this.avatar = 'data:image/png;base64,' + data.object.anh;
  ...
  ngOnInit(): void {
    ...
    console.log("detail", this.nv.id);
    this.getData(this.nv.id);
  }

  public setGioiTinh(){
    console.log("name",this.nv.gioi_tinh);
  ...
  public getData(id){
    ...
    this.nvService.getThongTinGiaDinh(id).subscribe(data=>{
      this.thongTinGiaDinh = data.list;
      console.log('gia đình', data.list);
    });
